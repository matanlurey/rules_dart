...
def make_dart_context(label,
  ...
  if not lib_root:
    if label.workspace_root.startswith("external/"):
      ...
      print("make_dart_context external %s" % lib_root)
    else:
...
def package_spec_action(ctx, dart_ctx, output):
  ...
  for dc in dart_ctxs:
    ...
    print("%s %s %s" % (dart_ctx.label.workspace_root, dc.package, dc.lib_root))
    if dart_ctx.label.workspace_root.startswith("external/"):
...
def layout_action(ctx, srcs, output_dir):
  ...
  """
  print("layout_action (output_dir: %s)" % output_dir)
  commands = []
  output_files = {}
  ...
  for src_file in srcs:
    print("Root: %s" % src_file.root)
    print("Short path: %s" % src_file.short_path)
